...
class RetailerSpider(Spider):
    ...
    start_urls = ['https://www.amazon.co.uk/Best-Sellers-Computers-Accessories-Laptops/zgbs/computers/429886031/ref=zg_m_bs_nav_computers_1']
    ...
    custom_settings = {
        ...
        'FEEDS': {
            # f'output/Amazon Products Detail {current_dt}.xlsx': {
            f'output/Amazon Products Detail {current_dt}.json': {
                # 'format': 'xlsx',
                'format': 'json',
                'encoding': 'utf8',
                # 'fields': [
                #     'Timestamp', 'Currency', 'Retailer Country', 'Retailer Website Address', 'Retailer Product ID',
                #     'product_page_url', 'Manufacturer EAN/UPC', 'Manufacturer Name', 'Brand Name', 'Category Name',
                #     'Category_Rank', 'Manufacturer Processor Name', 'Manufacturer Processor Brand Name', 'Operating Platform Name',
                #     'Software Included', 'Out of Stock', 'Price', 'Promotion Details', 'Product Title', 'Product Description',
                #     'Badges', 'Bullet Points', 'Logos in Bullet Points', 'Primary Image',  'Secondary Images', 'Image Sequence',
                #     'Complementary Products (Cross-Sells and Upsells)', 'Product Recommendations', 'Stock Availability (Click-and-Collect)',
                #     'Product Page Screenshot','Technical Specifications (e.g., Processor)','video_url']
                'fields': [
    ...
    def __init__(self):
        ...
        self.logs_filepath = f'logs/Amazon_logs {self.current_dt}.txt'
    ...
    def start_requests(self):
        # test code
        # url = 'https://www.amazon.co.uk/Lenovo-IdeaPad-Slim-i3-N305-Windows/dp/B0CZXTDMFG/ref=zg_bs_g_429886031_d_sccl_22/261-2453175-0303364?psc=1'
        # url = 'https://www.amazon.co.uk/Lenovo-14IRH8-Laptop-Intel-512GB/dp/B0CNRX88F2'
        # yield Request(url=url, callback=self.parse_details, meta={'page': 'Indexing Page'})
    ...
    def get_images(self, response, key):
        ...
        if key=='primary_img':
            ...
            # image_sequence = {ordinal(i + 1): url for i, url in enumerate(images_list)}  # for dict format
    ...
    def post_to_api(self, item):
        ...
        try:
            ...
            if api_res.status_code == 200:
                ...
                self.write_logs(f'[POST_TO_API] Successfully uploaded data for product ID {item.get("product_id")}. Response: {api_res.json()}')
                a=1
            else:
